...
class ReferenceNeighborhoodWeights:
...
def reference_neighborhood_score(
    query: dict,
    candidate: dict,
    weights: ReferenceNeighborhoodWeights = ReferenceNeighborhoodWeights(),
) -> dict:
    ...
    return {
        "reference_neighborhood_similarity": float(total),
        "ligand_similarity": float(ligand),
...
def similarity_bin(score: float, low_max: float = 0.35, mid_max: float = 0.65) -> str:
    if score <= low_max:
        return "low"
    if score <= mid_max:
        return "mid"
