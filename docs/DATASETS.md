...
The study uses public docking, cofolding, and protein-ligand benchmark resources. The repository does not redistribute raw structural archives or third-party model outputs. Instead, it stores lightweight derived source tables and manifests sufficient to verify the manuscript-level results.
...
`data/source_tables` contains compact CSV, Markdown, TeX, JSON, and manifest files no larger than 2 MB each. These files preserve the paper-facing denominators, split summaries, calibration outputs, reliability metrics, ablation summaries, external benchmark summaries, statistical correction tables, and case-taxonomy records.

## Excluded Large Tables

Large candidate-level prediction tables are listed in `data/source_tables/EXCLUDED_LARGE_TABLES.md`. These tables are omitted to keep the repository lightweight. They are derived artifacts rather than private raw data. The included summary tables and checksum manifest reproduce the manuscript-level claims.
